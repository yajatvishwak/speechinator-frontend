<script>
  ...
  let sentences;
  $: {
    if (script) sentences = script.split(".");
  }
</script>
...
>
  ...
  {#if step === 0}
    <ScriptLoading bind:script bind:step />
  {/if}
  {#if step === 1}
    <GrammarCheck bind:script bind:step bind:sentences />
  {/if}
  {#if step === 2}
    <Practice bind:script bind:step bind:sentences />
  {/if}
</section>
